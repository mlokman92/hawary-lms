 ...
 *
 * **Who sees what is not decided here.** `appointments: admin all, own
 * instructor, own student` is `app.is_admin OR app.owns_instructor OR
 * app.owns_student`, so a trainer's query comes back holding only her own
 * sessions however she sets the controls. This page used to *seed* her record
 * into the instructor filter and leave "All instructors" one click away, which
 * read as a rule but was only a default — the rows were hers because she had
 * not touched a dropdown, not because the server would not send the others.
 * The filter is admin-only now, which is the honest thing to show: a control
 * that cannot change the result is worse than no control.
 *
...
export function AppointmentListPage() {
  ...
  // What is still to come, nearest first — for everybody, admins included.
  // A register is opened to find a session, and the session somebody is about
  // to do something about is almost always ahead of them.
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-2">
        ...
        {/* Upcoming or archive, and nothing in between. "Any date" used to sit
            here as a third option and was the admin default, which put a
            session from last March at the top of a list opened to find out
            what is coming. */}
        <Select
          value={when}
          onValueChange={(v) => setWhen(v as AppointmentWhen)}
        >
        ...
        {/* Admin-only, because it is the only role for whom it can change
            anything: RLS sends a trainer her own sessions and nobody else's,
            so on her screen every option but one would return nothing. An
            admin who also teaches still wants "My sessions" here. */}
        {isAdmin ? (
          <Select
